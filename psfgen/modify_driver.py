...
def clean_src():
    subprocess.run(["make", "-C", "psexe", "XM_BUILTIN=true", "XMPLAY_VARIANT=redriver2", "clean"], check=True)


def _make_psflib_elf(
    xm: str,
    xm_dir: str = "retail",
    xmplay_variant: Optional[str] = "sbspss",
    xmplay_enable_fixes: Optional[bool] = False,
) -> lief.ELF.Binary:
    subprocess.run(
        [
            "make",
            ...
            "XMPLAY_ENABLE_FIXES=true" if xmplay_enable_fixes else "",
            f"XM_DIR={xm_dir}",
            f"XM={xm}",
        ],
        check=True,
...
def make_psflib_psf(exe: lief.ELF.Binary):
    with ps_exe.elf_to_psexe(exe) as p:
        psf1 = psf.PSF1()
        psf1.program = p.read()
    return psf1
...
def make_psflib(
    xm: str, xm_dir: str = "retail", xmplay_variant: Optional[str] = "sbspss"
) -> tuple[lief.ELF.Binary, psf.PSF1]:
    exe = _make_psflib_elf(xm, xm_dir, xmplay_variant)
    return exe, make_psflib_psf(exe)


def make_patched_songinfo(
    exe: lief.ELF.Binary, type: XMType, loop: bool, position: int, panning_type: XMPanningType
) -> tuple[lief.ELF.Symbol, SongInfoStruct]:
    # (pyright) lief has bad types
    song_info: lief.ELF.Symbol = exe.get_symbol("song_info")  # pyright: ignore[reportAssignmentType]
    ...
    info_struct = SongInfoStruct.from_buffer_copy(
        exe.get_content_from_virtual_address(song_info.value, sizeof(SongInfoStruct))
    )
...
def make_psf(
    ...
    loop: bool,
    position: int,
    panning_type: XMPanningType,
    xm_dir: str = "retail",
    xmplay_variant: Optional[str] = "sbspss",
    worse_timing: Optional[bool] = False,
) -> tuple[lief.ELF.Binary, psf.PSF1]:
...
def make_minipsf(
    lib: lief.ELF.Binary, lib_fn: str, type: XMType, loop: bool, position: int, panning_type: XMPanningType
):
    psf1 = psf.PSF1()
    ...
    psf1.program += ps_exe.PSXExeHeader(song_info.value, sizeof(SongInfoStruct))
